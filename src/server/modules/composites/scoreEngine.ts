...
export function buildCompositeScores(metrics: NormalizedMetrics): CompositeScore[] {
  const growth = buildScore(
    ...
    "Growth Score",
    [
      { metric: getMetric(metrics.growth, "ism-mfg-pmi"), weight: 1 / 11 },
      { metric: getMetric(metrics.growth, "ism-svc-pmi"), weight: 1 / 11 },
      { metric: getMetric(metrics.growth, "ip-yoy"), weight: 1 / 11 },
      { metric: getMetric(metrics.growth, "retail-sales-yoy"), weight: 1 / 11 },
      { metric: getMetric(metrics.growth, "init-claims-4w"), weight: 1 / 11, invert: true },
      { metric: getMetric(metrics.growth, "unemp-rate"), weight: 1 / 11, invert: true },
      { metric: getMetric(metrics.growth, "bldg-permits"), weight: 1 / 11 },
      { metric: getMetric(metrics.growth, "durable-goods-yoy"), weight: 1 / 11 },
      { metric: getMetric(metrics.growth, "capacity-util"), weight: 1 / 11 },
      { metric: getMetric(metrics.growth, "nfp-yoy"), weight: 1 / 11 },
      { metric: getMetric(metrics.growth, "jolts-openings"), weight: 1 / 11 },
    ],
  ...
  const inflation = buildScore(
    ...
    "Inflation Score",
    [
      { metric: getMetric(metrics.inflation, "ppi-final-demand-yoy"), weight: 0.10 },
      { metric: getMetric(metrics.inflation, "cpi-core-yoy"), weight: 0.15 },
      { metric: getMetric(metrics.inflation, "pce-core-yoy"), weight: 0.15 },
      { metric: getMetric(metrics.inflation, "supercore"), weight: 0.10 },
      { metric: getMetric(metrics.inflation, "ahe-yoy"), weight: 0.10 },
      { metric: getMetric(metrics.inflation, "breakeven-2y"), weight: 0.20 },
      { metric: getMetric(metrics.inflation, "breakeven-5y5y"), weight: 0.20 },
    ],
  ...
  const liquidity = buildScore(
    ...
    "Liquidity Score",
    [
      { metric: getMetric(metrics.liquidity, "hy-oas"), weight: 0.20 },
      { metric: getMetric(metrics.liquidity, "ig-oas"), weight: 0.16 },
      { metric: getMetric(metrics.liquidity, "sofr-ff-spread"), weight: 0.12 },
      { metric: getMetric(metrics.liquidity, "sofr-iorb-spread"), weight: 0.10 },
      { metric: getMetric(metrics.liquidity, "walcl-yoy"), weight: 0.12, invert: true },
      { metric: getMetric(metrics.liquidity, "sloos"), weight: 0.12 },
      { metric: getMetric(metrics.liquidity, "m2-yoy"), weight: 0.10, invert: true },
      { metric: getMetric(metrics.liquidity, "reserve-balances"), weight: 0.08, invert: true },
    ],
  ...
  const risk = buildScore(
    ...
    "Risk Sentiment",
    [
      { metric: getMetric(metrics.risk, "vix"), weight: 1 / 8, invert: true },
      { metric: getMetric(metrics.risk, "move-index"), weight: 1 / 8, invert: true },
      { metric: getMetric(metrics.risk, "stl-fsi"), weight: 1 / 8, invert: true },
      { metric: getMetric(metrics.risk, "credit-card-delinq"), weight: 1 / 8, invert: true },
      { metric: getMetric(metrics.risk, "hyg-ief-ratio"), weight: 1 / 8 },
      { metric: getMetric(metrics.risk, "market-breadth"), weight: 1 / 8 },
      { metric: getMetric(metrics.risk, "pct-spx-200dma"), weight: 1 / 8 },
      { metric: getMetric(metrics.risk, "cyc-def-ratio"), weight: 1 / 8 },
    ],

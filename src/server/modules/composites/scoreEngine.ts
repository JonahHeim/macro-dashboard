...
export function buildCompositeScores(metrics: NormalizedMetrics): CompositeScore[] {
  const growth = buildScore(
    ...
    "Growth Score",
    [
      { metric: getMetric(metrics.growth, "ism-mfg-pmi"), weight: 1 / 9 },
      { metric: getMetric(metrics.growth, "ism-svc-pmi"), weight: 1 / 9 },
      { metric: getMetric(metrics.growth, "ip-yoy"), weight: 1 / 9 },
      { metric: getMetric(metrics.growth, "retail-sales-yoy"), weight: 1 / 9 },
      { metric: getMetric(metrics.growth, "init-claims-4w"), weight: 1 / 9, invert: true },
      { metric: getMetric(metrics.growth, "unemp-rate"), weight: 1 / 9, invert: true },
      { metric: getMetric(metrics.growth, "bldg-permits"), weight: 1 / 9 },
      { metric: getMetric(metrics.growth, "durable-goods-yoy"), weight: 1 / 9 },
      { metric: getMetric(metrics.growth, "capacity-util"), weight: 1 / 9 },
    ],
  ...
  const inflation = buildScore(
    ...
    [
      ...
      { metric: getMetric(metrics.inflation, "supercore-proxy"), weight: 0.10 },
